(() => {
    // 3 x 3のタイルを想定。3x3にしたい場合はここを3にする
    const SQUARE_ROOT = 4;
    ...
    const handleClick = (_event) => {
        ...
        const target = _event.target; // 型アサーションするため引数のeventは_eventにする
        ...
        const isEmptyAdjacent = Object.values(everyDirectionElements).some((el) => el && el.textContent === "");
        if (!isEmptyAdjacent)
            return; // からのタイルに隣接していない場合は何もしない
        const emptyAdjacentElement = Object.values(everyDirectionElements).find((el) => el && el.textContent === "");
    ...
    for (let i = 0; i < TILE_COUNT; i++) {
        ...
        tileElement.dataset.index = i.toString();
